...
```diff
- var/something = 1
+ var/something = 2 // WOD13 EDIT CHANGE - ORIGINAL: var/something = 1
```
...
```byond
var/something = 2 // WOD13 EDIT CHANGE - ORIGINAL: var/something = 4
```
...
And then you'll want to establish your core folder that you'll be working out of which is normally your module ID. E.g. `wod13/modules/shuttle_toggle`
...
- **_Example:_** You're adding a new lavaland mob.
  ...
  First of all you create your modular folder. E.g. `wod13/modules/lavalandmob`
  ...
  ```byond
    /mob/lavaland/newmob
      icon = 'wod13/modules/lavalandmob/icons/mob.dmi'
      icon_state = "dead_1"
      sound = 'wod13/modules/lavalandmob/sounds/boom.ogg'
  ```
...
Example: `code/modules/mob/living/living.dm` -> `wod13/master_files/code/modules/mob/living/living.dm`
...
In short, most of the modular code will be placed in the subfolders of your main module folder **`wod13/modules/yourmodule/code/`**, with similar rules as with the assets. Do not mirror core code folder structures inside your modular folder.

For example, `wod13/modules/xenoarcheaology/code` containing all the code, tools, items and machinery related to it.
...
In general they are a good idea and encouraged whenever it is possible to do so. However this is not a hard rule, and sometimes WoD13 edits are preferable. Just try to use your common sense about it.
...
Performance should also be considered when you are overriding a hot proc (like Life() for example), as each additional call adds overhead. WOD13 edits are much more performant in those cases. For most procs this won't be something you have to think about, though.
...
- **Addition:**
  ...
  ```byond
  // WOD13 EDIT ADDITION START - SHUTTLE_TOGGLE - (Optional Reason/comment)
  var/adminEmergencyNoRecall = FALSE
  var/lastMode = SHUTTLE_IDLE
  var/lastCallTime = 6000
  // WOD13 EDIT ADDITION END
  ```
...
- **Removal:**
  ...
  /* // WOD13 EDIT REMOVAL START - SHUTTLE_TOGGLE - (Optional Reason/comment)
  for(var/obj/docking_port/stationary/S in stationary)
    if(S.id = id)
      return S
  */ // WOD13 EDIT REMOVAL END
  ...
  *Please avoid this however, only to be done if there is no other option.
  ...
  ```byond
  /* // WOD13 EDIT REMOVAL START - SHUTTLE_TOGGLE - (Moved to wod13/shuttle_toggle/randomverbs.dm)
...
- **Change:**
  ...
  ```byond
  if(SHUTTLE_STRANDED, SHUTTLE_ESCAPE, SHUTTLE_DISABLED) // WOD13 EDIT CHANGE - ORIGINAL: if(SHUTTLE_STRANDED, SHUTTLE_ESCAPE)
  ```
  ...
  ```byond
  /* // WOD13 EDIT REMOVAL START - Adds conditional
  	return 1
  */ // WOD13 EDIT REMOVAL
  // WOD13 EDIT ADDITION START - Adds conditional
  	if(!isnull(src))
  		return 1
  // WOD13 EDIT ADDITION END
...
That folder is **`code/__DEFINES/~wod13_defines`**, in which you can add them to the existing files, or create those files as necessary.
...
- Good: /wod13/modules/example_module/code/disease_mob.dm
- Bad: /wod13/modules/example_module/code/modules/antagonists/disease/disease_mob.dm

**Icons**: Any .DMI files must go in here.

- Good: /wod13/modules/example_module/icons/mining_righthand.dmi
- Bad: /wod13/modules/example_module/icons/mob/inhands/equipment/mining_righthand.dmi
...
ALL of the tgui files are located in `/tgui/packages/tgui/interfaces` and its subdirectories; there is no specific folder for World of Darkness 13 UIs.
...
You can do both `// WOD13 EDIT` and `/* WOD13 EDIT */`, though in some cases you may have to use one over the other.
...
<Button
	onClick={() => act('spin', { high_quality: true })}
	// WOD13 EDIT ADDITION START - another example, multiline changes
	icon="rat"
...
This way they are easily identifiable as modular TGUI .tsx/.jsx files. You do not have to do anything further, and there will never be any need for a WoD 13 edit comment in a modular TGUI file.
