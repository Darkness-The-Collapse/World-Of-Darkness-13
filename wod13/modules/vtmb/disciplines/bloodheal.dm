...
/datum/discipline_power/bloodheal/activate()
	...
	//aggravated damage
	owner.heal_ordered_damage(HEAL_AGGRAVATED * vitae_cost, list(BURN))

	//brain damage and traumas healing
	var/obj/item/organ/brain/brain = owner.get_organ_slot(ORGAN_SLOT_BRAIN)
	if(brain)
		brain.apply_organ_damage(-HEAL_BASHING_LETHAL * vitae_cost)
	...
	if(eyes)
		eyes.apply_organ_damage(-HEAL_BASHING_LETHAL * vitae_cost)

		owner.adjust_temp_blindness(-2*HEAL_AGGRAVATED * vitae_cost SECONDS)
		owner.adjust_eye_blur(-2*HEAL_AGGRAVATED * vitae_cost SECONDS)

	var/obj/item/organ/ears/ears = owner.get_organ_slot(ORGAN_SLOT_EARS)
	if(ears)
		ears.apply_organ_damage(-HEAL_AGGRAVATED * vitae_cost)

	var/obj/item/organ/heart/heart = owner.get_organ_slot(ORGAN_SLOT_EARS)
	if(heart)
		heart.apply_organ_damage(-HEAL_BASHING_LETHAL * vitae_cost)

	var/obj/item/organ/lungs/lungs = owner.get_organ_slot(ORGAN_SLOT_LUNGS)
	if(lungs)
		lungs.apply_organ_damage(-HEAL_BASHING_LETHAL * vitae_cost)

	var/obj/item/organ/stomach/stomach = owner.get_organ_slot(ORGAN_SLOT_STOMACH)
	if(stomach)
		stomach.apply_organ_damage(-HEAL_BASHING_LETHAL * vitae_cost)

	var/obj/item/organ/liver/liver = owner.get_organ_slot(ORGAN_SLOT_LIVER)
	if(liver)
		liver.apply_organ_damage(-HEAL_BASHING_LETHAL * vitae_cost)
...
/datum/discipline_power/bloodheal/proc/adjust_vitae_cost()
	...
	var/total_aggravated_damage = owner.getFireLoss()
