...
/mob/living/simple_animal/hostile/gargoyle/proc/gain_nigs()
	...
	var/list/candidates = SSpolling.poll_ghosts_for_targets(
		question = "Do you want to play as Embraced Gargoyle?",
...
/datum/action/gargoyle/Trigger()
	...
	G.adjust_brute_loss(-300)
	G.adjust_fire_loss(-300)
	G.Stun(50)
	G.petrify(50)
...
// Removes due to outdate (must be fixed)
/mob/living/carbon/human/proc/switch_masquerade(mob/living/carbon/human/H)
	...
	if(!hided)
		hided = TRUE
		if(masquerade)
			clane.violating_appearance = FALSE
