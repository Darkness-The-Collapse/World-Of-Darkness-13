...
/datum/action/blood_power/apply_button_icon(atom/movable/screen/movable/action_button/current_button, force = FALSE)
	if(owner)
		if(owner.client)
...
/datum/action/blood_power/Trigger(mob/clicker, trigger_flags)
...
/datum/action/give_vitae
	...
	button_icon = 'wod13/modules/vtmb/icons/actions.dmi'
	check_flags = AB_CHECK_HANDS_BLOCKED|AB_CHECK_IMMOBILE|AB_CHECK_LYING|AB_CHECK_CONSCIOUS
	vampiric = TRUE
	var/giving = FALSE

/datum/action/give_vitae/Trigger(mob/clicker, trigger_flags)
	if(istype(owner, /mob/living/carbon/human))
					...
					if((BLOODBONDED.timeofdeath + 5 MINUTES) > world.time)
						...
						if (iskindred(BLOODBONDED) && save_data_v)
							...
							if(H.generation < 13)
								...
								BLOODBONDED_prefs_v.write_preference(GLOB.preference_entries[/datum/preference/choiced/skin_tone], get_vamp_skin_color(BLOODBONDED.skin_tone))
...
/datum/action/blush_of_life/Trigger(mob/clicker, trigger_flags)
	if(!istype(owner, /mob/living/carbon/human))
		return
	...
	if(!blusher.active_blush)
		blusher.skin_tone = initial(blusher.skin_tone)
		blusher.update_body()
		blusher.active_blush = TRUE
	else
		blusher.skin_tone = get_vamp_skin_color(initial(blusher.skin_tone))
		blusher.update_body()
...
/mob/living/carbon/human/update_body(is_creating = FALSE)
	. = ..()
	if (!clane || !iskindred(src))
		return

	//deflate people if they're super rotten
	if ((clane.alt_sprite == "rotten4") && (base_body_mod == "f"))
		base_body_mod = ""
...
/obj/item/organ/brain/zombie/vampire
